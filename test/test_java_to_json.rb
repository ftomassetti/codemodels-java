...
class TestJavaToJson < Test::Unit::TestCase
  ...
  def test_model_contains_class
    assert_not_nil @example_basic.only_child_deep_of_eclass(@eclass_class)
  end

  def test_myclass_has_right_name    
    my_class = @example_basic.only_child_deep_of_eclass(@eclass_class)
  ...
  def test_fields_are_found
    $nextId = 1
    js = JavaModel.jsonize_java_obj(@example_accessors.only_child_deep_of_eclass(@eclass_class))
  ...
  def test_methods_are_found
    $nextId = 1
    my_class = @example_accessors.only_child_deep_of_eclass(@eclass_class)
    js = JavaModel.jsonize_java_obj(my_class)
